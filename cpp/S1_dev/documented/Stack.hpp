...
/* Стек - структура данных, работающая
 по принципу LIFO (Last In, First Out),
 т.е. элементы извлекаются из стека в порядке,
 обратном порядку их добавления.
...
public:
    ...
    /* деструктор освобождает память, выделенную под список
    (вызывается неявно при уничтожении объекта);
     кода объект содержит в себе какой-либо внешний ресурс
    (например, динамически выделенную память),
     необходимо(!) реализовать (или запретить) конструктор
    копирования, оператор присвивания и деструктор,
     в C++11 желательно также определить конструктор перемещения
...
// конструктор
template <class T>
Stack<T>::Stack() : head_(nullptr) // nullptr - пустой указатель
                                   // (не указывает ни на какой адрес)
